...
CREATE TABLE IF NOT EXISTS `crowdcontrol`.`Experiment` (
  ...
  `anwers_per_worker` INT NULL,
  `ratings_per_worker` INT NULL,
  `answer_type` VARCHAR(191) NULL DEFAULT NULL,
...
USE `crowdcontrol`$$
CREATE DEFINER = CURRENT_USER TRIGGER `crowdcontrol`.`Chosen_Task_Chooser_Param_BEFORE_INSERT` BEFORE INSERT ON `Chosen_Task_Chooser_Param` FOR EACH ROW
BEGIN
...
USE `crowdcontrol`$$
CREATE DEFINER = CURRENT_USER TRIGGER `crowdcontrol`.`Chosen_Answer_Quality_Param_BEFORE_INSERT` BEFORE INSERT ON `Chosen_Answer_Quality_Param` FOR EACH ROW
BEGIN
	DECLARE msg VARCHAR(191);
	DECLARE checkregex VARCHAR(191);
...
USE `crowdcontrol`$$
CREATE DEFINER = CURRENT_USER TRIGGER `crowdcontrol`.`Chosen_Rating_Quality_Param_BEFORE_INSERT` BEFORE INSERT ON `Chosen_Rating_Quality_Param` FOR EACH ROW
BEGIN
	DECLARE msg VARCHAR(191);
	DECLARE checkregex VARCHAR(191);
