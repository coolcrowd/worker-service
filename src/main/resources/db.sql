...
CREATE TABLE IF NOT EXISTS `crowdcontrol`.`Experiments_Platform_Mode` (
  ...
  `experiments_platform` INT NOT NULL,
  `mode` ENUM('answer', 'rating', 'disabled') NOT NULL,
  `timestamp` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
-- -----------------------------------------------------
-- Table `crowdcontrol`.`Worker_Token`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `crowdcontrol`.`Worker_Token` (
  `token` VARCHAR(191) NOT NULL,
  `worker_id` INT NOT NULL,
  PRIMARY KEY (`token`),
  UNIQUE INDEX `token_UNIQUE` (`token` ASC),
  INDEX `token_for_ud_idx` (`worker_id` ASC),
  CONSTRAINT `token_for_ud`
    FOREIGN KEY (`worker_id`)
    REFERENCES `crowdcontrol`.`Worker` (`id_worker`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
